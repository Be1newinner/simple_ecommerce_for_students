...
describe("JWT UTILS FUNCTIONS", () => {
    ...
    it("should throw an error, since, JWT_SECRET ENV is missing", async () => {
        ...
        await expect(createToken({ ...mockUser, expiry: "1h" })).rejects.toThrow("JWT_SECRET NOT FOUND");
    })

    it("should throw an error, since, payload is missing ENV is missing", async () => {
        const localMockUser = { ...mockUser };
        delete localMockUser.role
        await expect(createToken({ ...localMockUser, expiry: "1h" })).rejects.toThrow("uid, email, role, expiry are required for token generation!");
    })
    ...
    it("GENERATE LOGIN TOKENS", async () => {
        jsonwebtoken.sign.
            mockResolvedValueOnce("mockAccessToken").mockResolvedValueOnce("mockRefreshToken");
    ...
    it("TO BE THROWN ERROR WHEN JWT_SECRET NOT FOUND", async () => {
        ...
        await expect(decryptToken("THE_TOKEN_IS_HERE")).rejects.toThrow("JWT_SECRET NOT FOUND OR INVALID");
    })
    ...
    it("TO BE THROWN ERROR WHEN TOKEN IS INVALID", async () => {
        ...
        await expect(decryptToken("invalid_Token")).rejects.toThrow("Invalid or expired token");
    })

    it("TO BE THROWN ERROR WHEN TOKEN IS NOT STRING", async ()=>{
        await expect(decryptToken(null)).rejects.toThrow("TOKEN IS REQUIRED AND MUST BE A STRING")
    })
